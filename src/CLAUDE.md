...
**IPC 境界のモック契約**: Rust コマンドの戻り値をモックするとき、Rust 側の不変条件を再現する。例: `scan_ghosts_with_meta` は `cache_hit=true` なら必ず `ghosts: []` を返す。`cacheHit: true, ghosts: [1件]` のような現実に発生しない組み合わせでモックすると、テストがバグを検知できなくなる。
