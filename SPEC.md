...
2. **キャッシュ判定**（`forceFullScan` でない場合）:
   ...
   2. DB にデータがあれば `localStorage` から fingerprint を取得（`getCachedFingerprint`）→ `cachedFingerprint` として Rust に送る
...
5. **キャッシュミス時**: スキャン結果を SQLite へ置換保存（`replaceGhostsByRequestKey`）し、fingerprint を `localStorage` へ更新
6. **寿命管理**: 世代超過・TTL 超過の `request_key` を SQLite から削除（`cleanupOldGhostCaches`）し、対応する fingerprint を `localStorage` から刈り込む（`pruneFingerprintCache`）
