...
- **計器を先に較正する**: 新しい診断手段（ログ・サンプラー・監視スクリプト・回帰検知ゲート）は、既知の事実を正しく映すことを確認してから証拠として採用する。計器の視界の欠損（ツール環境のプロセス可視性制限等）は「存在するもの」を「存在しない」と誤認させる。**ゲートは「通ること」でなく「壊れたツリーで落ちること」で較正する** — 決して落ちないゲートは、機能しているゲートと区別がつかない
- **コマンドの成功を、検査対象が走った証拠と読まない**: ビルドツールはキャッシュで処理を省く。`cargo` が成功しても build script や rustc が再実行されたとは限らず、その出力を grep して 0 件でも「無い」証拠にはならない（本 repo では、`dist` 不在でも build script が走らず成功した件と、`-C panic=abort` の有無を検算しようとして rustc 行が 1 つも出なかった件で観測）。実行を強制する手（成果物ディレクトリの削除・入力ファイルの touch）を打ち、走った事実を別経路（出力ファイルの更新時刻等）で確かめてから読む
- **確立したツールの統計判定も較正の対象に含める**: criterion の `p = 0.00 < 0.05` のような有意差判定は「同一条件の反復なら差は出ない」という前提の上に立つ。この前提が成り立つかは環境依存で、成り立たなければ判定はそのまま誤る。**同一物を 2 回測る**ことでノイズの振れ幅を先に測り、主張したい効果量がそれを超えるか確かめる（本 repo の criterion 計測では、同一バイナリ同士で −28.5%・`p = 0.00` が出た実績がある）。単発の数字で速い/遅いを言わない
...
GitHub Flow。ブランチ名は `{prefix}/{issue番号}-{英語で内容の説明}`（プレフィックスは `feature/` `fix/` `hotfix/` `release/` `test/` `docs/` `refactor/` `chore/`。番号のみの `fix/15` は不可）。issue を起点としない作業（依存更新・振り返り等）は issue 番号を省き `{prefix}/{英語で内容の説明}` とする。着手・PR・マージ後の手順は `/start-issue`・`/commit`・`/pr`・`/post-merge-sync` が単一権威。
