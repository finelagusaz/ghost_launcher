...
- **完了させる issue**: この PR で受け入れ基準を満たす issue を洗い出し、本文に `Closes #N` を書く。設計書・計画書が「フェーズ完了時に issue をクローズする」と定めている場合も同様（文書内のクローズ指示は機械検証されないため、PR 本文への配線が唯一の自動化点。epic #140 で #135/#136 のクローズ漏れが起きた教訓）
