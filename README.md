...
Ghost Launcher は、手持ちのゴーストをずらりと一覧にして、名前を数文字打つだけで呼び出せるデスクトップランチャーです。
...
- **すばやい検索** — ゴースト名・作者名の部分一致で検索。全角/半角の表記揺れも吸収し、IME で変換している途中に検索が走ることもありません
- **キーボードだけで起動** — 起動直後から検索欄に入力でき、`↑` `↓` で選んで `Enter` を押すだけ。マウスいらずです
- **ソート & ランダム起動** — 名前順・最近起動順・起動回数順・ランダム順を切り替え。迷ったらランダム起動ボタンで一体お呼びできます
...
- Windows
- [SSP](https://ssp.shillest.net/) がインストール済みであること（`ssp.exe` を使ってゴーストを起動します）
- WebView2 ランタイム
...
もちろん、一覧のカードにある「起動」ボタンをクリックしても起動できます。

## カスタマイズ

UI の文言はお好みに書き換えられます。実行ファイルと同じフォルダに `locales/{言語}.json` を置いてください。
